...
Start with the runnable [CLI quick start](../../examples/cli-quickstart/README.md). The commands below are the reference surface after the workspace packages have been built.
...
Exit codes in the current alpha are `0` for success/PASS, `2` for REVIEW, `3` for BLOCKED, and `1` for invalid input or command failure. See the [Integration Guide](../../docs/INTEGRATION_GUIDE.md) for result handling and the [protocol reference](../../docs/PROTOCOL.md) for complete request objects.
