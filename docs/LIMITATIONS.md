...
The filesystem receipt store uses fsynced staging files and atomic no-replace hard-link publication per run ID, nonce, transaction, receipt digest, and consumption event. It does not use a stealable stale global lock. Multi-file issuance is not one filesystem transaction. Pre-commit crash cleanup is allowed only during an operator-enforced offline/exclusive maintenance window with the exact transaction ID; running recovery beside a live issuer is unsupported. Directory fsync is attempted and fails closed except on platforms that explicitly report it unsupported, so ultimate power-loss durability still depends on filesystem and operating-system guarantees. It is not a distributed transaction system and assumes every consumer uses the same protected store on one filesystem. Copying or restoring older store state can restore previously consumed state.
